## Q. Write a query that'll identify returning active users. A returning active user is a user that has made a second purchase within 7 days of any other of their purchases.
...
```sql
with cte as (select user_id, created_at, lag(created_at)  over(partition by user_id order by created_at) 
as prev_transaction from amazon_transactions)
select DISTINCT user_id from cte where datediff(created_at,prev_transaction) <= 7;
```
