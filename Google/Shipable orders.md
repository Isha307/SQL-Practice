## Q. Find the percentage of shipable orders.
...
```sql
select (select count(*) from orders o left join customers c on o.cust_id=c.id 
where c.address is not null)*100.0/count(*) as percent_shipable 
